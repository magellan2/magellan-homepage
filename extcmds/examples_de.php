...
<body>
    ...
    ?>
      <div id="center">
            <h2>Beispiele</h2>
            <p>Ich hab hier eine Reihe von Beispielen untergebracht. Sie stammen nicht alle von mir.
                Wer einen besonders coolen Script hat und ihn der Welt kundtun will, soll sich bei
                mir melden. Schreib mir ne Mail an thoralf at m84 de.</p>
            <p>
            ...
            <p>Hier ein Beispiel, wie man ganz einfach eine Einheit bestätigt. Wird der Script
...
if (!unit.isOrdersConfirmed()) {
  unit.setOrdersConfirmed(true);
            ...
            <p>
                Die Bedingung
                <tt>unit.isOrdersConfirmed()</tt>
                ist immer dann hilfreich, wenn man Befehle nicht doppelt geben will. Zum Beispiel,
                wenn aus Versehen der Script doppelt ausgeführt wird.
                <tt>unit</tt>
                ist dabei das Objekt der Einheit und enthält alle Informationen über diese Einheit
                und indirekt über die gesamte aktuell geladene Welt. Dazu aber später mehr.
            </p>
            <p>
                Mit
                <tt>unit.setOrdersConfirmed()</tt>
                wird dann die Einheit bestätigt, so wie man es von Magellan mit der
                Tastenkombination STRG+B kennt.
            </p>
            <hr />
            <a name="script2"></a>
            <h3>Pferdedressur</h3>
            <p>Mit Hilfe des folgenden Scripts kann man einer Einheit, die das Talent Pferdedressur
                besitzt sagen, dass sie nur dann Pferde aus der Region dressieren soll, wenn es mehr
                als 50 Pferde in der aktuellen Region gibt. Ansonsten wird einfach Pferdedressur
                gelernt.</p>
            <pre class="example">
...
if (!unit.isOrdersConfirmed()) {
  ...
  if (helper.getItemCount(unit,"Pferd")==0) {
    unit.setOrdersConfirmed(true);
            ...
            <p>Der folgende Script überprüft, ob die aktuelle Einheit die Einheit mit der Nummer
                "i2de" sieht - sprich, ob sie sich in der Region befindet. Wenn dem so ist, übergibt
                sie alle Kräuter, die sie besitzt, dieser Einheit. Im Falle von i2de handelt es sich
                um einen Transporter, der alle Kräuter aufsammelt und von A nach B transportiert
                (siehe unten).</p>
            <pre class="example">
...
if (!unit.isOrdersConfirmed()) {
  if (helper.unitSeesOtherUnit("i2de")) {
    ...
    unit.setOrdersConfirmed(true);
            ...
            <p>
                Hier ist
                <tt>helper.addOrder()</tt>
                neu. Damit wird an den aktuellen Befehl der Einheit eine neue Zeile hinzugefügt.
            </p>
            <hr />
            <a name="script4"></a>
            <h3>Transporter automatisieren</h3>
            <p>Dieser Script ist ganz hilfreich für die Entwicklung eigener Scripte für Transporter,
                die zwischen bestimmten Regionen pendeln und pro Region bestimmte Dinge entladen.</p>
            <pre class="example">
...
if (!unit.isOrdersConfirmed()) {
  ...
  // ------- Rolkos --------
    ...
    unit.setOrdersConfirmed(true);
            ...
            <p>Dieser Script ist ganz nützlich, wenn man einen Produzenten hat, der Sachen herstellt
...
if (!unit.isOrdersConfirmed()) {
  ...
  unit.setOrdersConfirmed(true);
            ...
            <p>Dies ist ein Script, der es einfach macht, Einheiten auszubilden und sich so wenig
...
soldier(Unit unit, int level, String talent, String waffe, 
  ...
  if (helper.getItemCount(unit,waffe)>0 && bewache) {
    helper.addOrder("BEWACHEN");
            ...
            <p>Hier ein Beispiel für den Aufruf der Funktion. Die angegebene Einheit soll
                ...
                langweilig ist, auch noch T5 Steuereintreiben.</p>
            <pre class="example">
...
if (!unit.isOrdersConfirmed()) {
  ...
  soldier(unit, 5, talent, waffe, taktiker , held , lerne , bewache);
            ...
            <p>Wenn man in einer Allianz arbeitet und die Reports untereinander teilt, dann ist es
...
clearRegion(Region region, String partei) {
  for (Unit unit : region.units()) {
    ...
    unit.setOrdersConfirmed(true);
            ...
            <p>Pro Region ruft man diese Funktion dann wie folgt auf.</p>
            <pre class="example">

clearRegion((Region)container,"ntc");
        </pre>
            <p>Prinzipiell könnte man auch eine Funktion pro Partei-Container schreiben. Das geht
                natürlich auch.</p>
            <hr />
            <a name="script8"></a>
            <h3>Händler automatisieren</h3>
            <p>Dieser Script ist etwas komplexer. Es handelt sich auch wieder um eine Funktion, die
                ...
                mich um nichts mehr kümmern - außer das gewonnen Silber ausgeben.</p>
            <pre class="example">
...
haendler(Unit haendler, int kaufenFaktor, Map transporters) {
  if (!haendler.isOrdersConfirmed()) {
    ...
    unit.setOrdersConfirmed(true);
            ...
            <p>Um diesen Script aufzurufen, muss man ihm drei Dinge übergeben. Das Objekt der
...
if (!unit.isOrdersConfirmed()) {
  ...
  haendler(unit, kaufenFaktor, transporters);
            ...
            <a name="script9"></a>
            <h3>Silber horten</h3>
            <p>Der folgende Befehl ist wieder einfach. Er zeigt aber, dass es möglich ist, die
...
if (!unit.isOrdersConfirmed()) {
  ...
  unit.setOrdersConfirmed(true);
            ...
            <p>Mit dem nachfolgenden Script kann man zu einer Liste von Regionen (nodeList ist eine
 ...
 if (region.maxLuxuries() > 0) {
   ...
   for (j = 0; j < region.getPrices().size(); j++) {
     ...
     region.addSign(sign);
            ...
            <p>Hiermit werden Schilder aufgestellt, die zeigen, wie viele Pferde in einer Region
                stehen.</p>
            <pre class="example">
